...
async function uploadToHostinger(localFile, remoteFile) {
  ...
  try {
    ...
    // Ensure the target folder exists before writing into it —
    // STOR fails with "550 No such file or directory" if it doesn't.
    const remoteDir = path.dirname(remoteFile);
    const remoteName = path.basename(remoteFile);
